...
public:
    ...
    T findMinimalElement() //макс элемент
    {
        Node *current = root;
    ...
    static Node* Deleting(Node* p, int data) // удаление элемента
    {
        if( !p ) return p;
        if( p->data==data )
        {
            ...
            p->l = Deleting(p->l,data);
        else
            p->r = Deleting(p->r,data);
        return p;
